...
import { fetchVideoStatusStats } from "@/store/slice/overview/Overview";
...
export default function VideoStatusChart() {
  const dispatch = useAppDispatch();
  const { videoStatusData, loading } = useAppSelector((s) => s.overview);

  const [period, setPeriod] = useState<TimePeriod>("CURRENT_MONTH");
  const [year, setYear] = useState(new Date().getFullYear());

  useEffect(() => {
    dispatch(fetchVideoStatusStats({ period, year }));
  }, [dispatch, period, year]);

  const chartData = Object.entries(videoStatusData?.countsByStatus ?? {}).map(
    ([status, count]) => ({
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">
              Video theo Trạng Thái
            </h3>
            <p className="text-xs text-muted-foreground">
              Tổng số video theo từng trạng thái
            </p>
          </div>
        </div>
        <PeriodYearFilter
          ...
          <BarChart data={chartData} barCategoryGap="30%">
            ...
            <Bar dataKey="count" radius={[6, 6, 0, 0]}>
              {chartData.map((entry, index) => (
                <Cell key={`vid-cell-${index}`} fill={entry.color} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
